...
def do(iqueue, cqueue): # pragma: no cover
    while True:
        ...
        with transaction.atomic():
            try:
                if sent.clean_parses['treeparse'] != 'null':
                    results = analyzers.ComplexityCleanAnalyzer(sent.clean_text, sent.clean_parses['treeparse']).analyze()

                    sent.metrics['clean_complexity'] = results
                    sent.save()
                else:
...
class ComplexityCleanTagger(taggers.Tagger):
    def __init__(self, settings, num_processes, sentObjects):
        super(ComplexityCleanTagger, self).__init__(settings, num_processes)
