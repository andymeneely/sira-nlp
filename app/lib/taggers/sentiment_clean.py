...
def do(iqueue, cqueue): # pragma: no cover
    while True:
        ...
        with transaction.atomic():
            try:
                results = analyzers.SentimentCleanAnalyzer(sent.clean_text, url).analyze()

                sent.metrics['clean_sentiment'] = results
                sent.save()
...
class SentimentCleanTagger(taggers.Tagger):
    def __init__(self, settings, num_processes, sentObjects):
        super(SentimentCleanTagger, self).__init__(settings, num_processes)
