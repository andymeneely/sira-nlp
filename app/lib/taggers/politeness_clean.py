...
def do(iqueue, cqueue): # pragma: no cover
    while True:
        ...
        with transaction.atomic():
            try:
                results = analyzers.PolitenessCleanAnalyzer(
                    sent.clean_text, sent.clean_parses['depparse'],
                ).analyze()
                sent.metrics['clean_politeness'] = results
                sent.save()
...
class PolitenessCleanTagger(taggers.Tagger):
    def __init__(self, settings, num_processes, sentenceObjects):
        super(PolitenessCleanTagger, self).__init__(settings, num_processes)
