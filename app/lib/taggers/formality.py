...
CLS = _pickle.load(FORMALITY_CLASSIFIER_PATH)
VEC = _pickle.load(FORMALITY_VECTORIZER_PATH)
...
class FormalityTagger(taggers.Tagger):
    def __init__(self, settings, num_processes, sentenceObjects):
        super(FormalityTagger, self).__init__(settings, num_processes)
